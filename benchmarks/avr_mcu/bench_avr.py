...
sys.path.insert(
    0, str(pathlib.Path(__file__).resolve().parents[1] / "sparse_mcu")
)
...
def _have_tools():
    return (
        shutil.which(AVR_GCC)
        and shutil.which(AVR_SIZE)
...
def _build_driver(workdir):
    ...
    cp = subprocess.run(
        ["gcc", str(DRIVER_SRC), "-lsimavr", "-o", str(drv)],
        capture_output=True,
        text=True,
    )
    if cp.returncode != 0:
...
def _build_and_run(qm, x_seq, sparse, driver, workdir):
    ...
    cp = subprocess.run(
        [
            ...
            *CFLAGS,
            "-I",
            str(workdir),
            str(workdir / "rc_kernel.c"),
            str(workdir / "main_bench.c"),
            "-o",
            str(elf),
        ],
        capture_output=True,
    ...
    sz = (
        subprocess.run(
            [AVR_SIZE, str(elf)], check=True, capture_output=True, text=True
        )
        .stdout.splitlines()[1]
        .split()
    )
    ...
    cp = subprocess.run(
        [str(driver), str(elf)], capture_output=True, text=True, timeout=300
    )
                    ...
                    if dtype in ("i8", "i16") and kernel in ("dense", "csr"):
                        try:
                            cfg = calibrate_from_data(
                                rc, exe, X[:900], storage_bits=BITS[dtype]
                            )
                            ...
                            out = np.asarray(
                                CompiledAffineRC(qm).predict(x_seq)
...
def main():
    ...
    if not _have_tools():
        print(
            "Need avr-gcc + avr-libc and host gcc + libsimavr-dev. Aborting."
        )
        return 1
    ...
    if args.json:
        args.json.write_text(
            json.dumps(dict(target="atmega328p", rows=rows), indent=2)
        )
