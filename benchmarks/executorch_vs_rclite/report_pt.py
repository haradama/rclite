...
def main() -> int:
    ...
    m55 = (json.loads((HERE / "fvp" / "out" / "m55_result.json").read_text())
           if (HERE / "fvp" / "out" / "m55_result.json").exists() else None)
    if m55:
        npu, cpu, rcl = (m55["executorch_ethos_u_int8"],
                         m55["executorch_cpu_float32"], m55["rclite_int8"])
        a("## Same target — all three on one Cortex-M55 (Corstone-300 FVP)")
        a("")
        a("Identical 80-unit ESN, identical FVP/CPU. ExecuTorch is shown both with "
        ...
        a("")
        a("| stack (same ESN, same M55) | engine | code (`.text`) | model + working RAM | CPU cycles/step¹ | NRMSE |")
        a("|---|---|--:|--:|--:|--:|")
        a(f"| ExecuTorch + Ethos-U55 | **NPU** (int8) | {npu['code_text_bytes']/1024:.0f} KB "
          f"| `.pte` {npu['pte_bytes']/1024:.1f} KB + arena {npu['arena_bytes']} B "
          f"| {npu['cpu_cycles_per_step']:,} (+{npu['npu_cycles_per_step']:,} NPU) | 41% (int8 PTQ) |")
        a(f"| ExecuTorch CPU-only | M55 CPU (float32) | {cpu['code_text_bytes']/1024:.0f} KB "
        ...
        a(f"| **rclite** | M55 CPU (int8) | **{rcl['code_text_bytes']/1024:.1f} KB** "
          f"*(whole firmware)* | **{rcl['ram_bytes']} B** | **{rcl['cpu_cycles_per_step']:,}** "
          f"| {q['i8_affine_qat']*100:.1f}% / {q['i16_affine_qat']*100:.2f}% (QAT) |")
        a("")
        a(f"On the **identical M55 CPU**, rclite is **~{cpu['code_text_bytes']/rcl['code_text_bytes']:.0f}× "
        ...
        a("")
        a("¹ FVP cycle model (rclite: M55 DWT CYCCNT; ExecuTorch: `arm_perf_monitor` "
        ...
        if fw:
            r8 = fw["rc_i8"]
            a(f"> rclite is not limited to the M55: the *same* ESN also runs on a bare "
