...
def _bench_row(kind, units, density):
    ...
    if kind == "symmetric":
        qm = quantize_model(rc, exe, QuantConfig(state_frac=16, input_frac=12,
                                                 weight_frac=12),
    ...
    return {
        "kind": kind, "N": N, "density": density, "nnz": nnz,
        "mac_reduction": (N * N) / max(nnz, 1),
        "t_dense_ms": td * 1e3, "t_sparse_ms": tsp * 1e3,
        "speedup": td / tsp if tsp else float("nan"),
        "exact": max_diff == 0, "max_diff": max_diff,
    }


def main():
    print("dense vs sparse W_res — quantized integer kernels (host JIT, T=2000)\n")
    header = (f"{'kernel':>10} {'N':>4} {'dens':>5} {'nnz':>6} {'MAC↓':>6} "
    ...
    for kind in ("symmetric", "affine"):
        for units, density in [(100, 0.1), (200, 0.1), (300, 0.1)]:
            ...
            print(f"{r['kind']:>10} {r['N']:>4} {r['density']:>5.2f} "
