...
def build_setup(
    N: int = 80, n_train: int = 2000, n_eval: int = 200, seed: int = 42
):
...
def bench_inference(rc, exe, X, Y, n_train, n_eval, n_runs=5):
    ...
    print(
        f"Reservoir: N={rc.reservoir.units}, K={rc.input.units}, "
    ...
    qm_i32_sym = quantize_model(
        rc, exe, cfg_i32_sym, target=I32FixedPoint(), lut=TanhLUTSpec(n=128)
    )
    cfg_i16_sym = QuantConfig(state_frac=10, input_frac=8, weight_frac=8)
    qm_i16_sym = quantize_model(
        rc, exe, cfg_i16_sym, target=I16FixedPoint(), lut=TanhLUTSpec(n=128)
    )
    cfg_i8_sym = QuantConfig(state_frac=5, input_frac=4, weight_frac=4)
    qm_i8_sym = quantize_model(
        rc, exe, cfg_i8_sym, target=I8Symmetric(), lut=TanhLUTSpec(n=32)
    )
    ...
    paths = [
        ...
        (
            "Python i32 sym",
            lambda: (qexe_i32_s.reset(), qexe_i32_s.predict(eval_X)),
        ),
        (
            "Python i16 sym",
            lambda: (qexe_i16_s.reset(), qexe_i16_s.predict(eval_X)),
        ),
        (
            "Python i8  sym",
            lambda: (qexe_i8_s.reset(), qexe_i8_s.predict(eval_X)),
        ),
        (
            "Python i16 affine",
            lambda: (qexe_i16_a.reset(), qexe_i16_a.predict(eval_X)),
        ),
        (
            "Python i8  affine",
            lambda: (qexe_i8_a.reset(), qexe_i8_a.predict(eval_X)),
        ),
    ...
    for name, best, mean in results:
        rel = best / fastest
        print(
            f"{name:<28} {fmt_us_per_step(best, n_eval)} {fmt_us_per_step(mean, n_eval)}  {rel:>4.1f}x"
        )
    print()
...
def bench_setup(rc, exe, X, n_train, n_runs=3):
    ...
    items = [
        (
            "calibrate i8",
            lambda: calibrate_from_data(rc, exe, X[:n_train], storage_bits=8),
        ),
        (
            "calibrate i16",
            lambda: calibrate_from_data(rc, exe, X[:n_train], storage_bits=16),
        ),
        ("quantize_model i8", lambda: quantize_model_affine(rc, exe, cfg_i8)),
        (
            "quantize_model i16",
            lambda: quantize_model_affine(rc, exe, cfg_i16),
        ),
    ]
    for name, fn in items:
        ...
        best, mean = time_call(fn, n_runs=n_runs)
        print(
            f"  {name:<22} best={best * 1000:6.1f} ms  mean={mean * 1000:6.1f} ms"
        )
    print()
...
def bench_qat(rc, exe, X, Y, n_train, n_eval, n_runs=2):
    ...
    print("=" * 70)
    print(
        f"QAT search cost (T_train={n_train}, T_eval={n_eval}, n_runs={n_runs})"
    )
    print("-" * 70)
    eval_X = X[n_train : n_train + n_eval]
