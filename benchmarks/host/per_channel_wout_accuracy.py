...
def _train(M, K, units, seed):
    rc = ReservoirComputer(
        ...
        reservoir=ReservoirNode(units=units, topology=Topology.ESN_STANDARD,
                                leak_rate=0.3, density=0.2, seed=seed,
                                name="res"),
    ...
    Y = np.stack([(m + 1) * 0.5 * np.sin(np.arange(1000) * 0.03 * (m + 1))
...
def _mse(rc, exe, X, Y, pc_out):
    cfg = calibrate_from_data(rc, exe, X[:750], storage_bits=8,
...
def main():
    print("per-tensor vs per-channel W_out — affine i8 quantized MSE "
          "(MIMO, heterogeneous-amplitude outputs)\n")
    header = (f"{'M':>3} {'K':>3} {'seed':>4} {'float MSE':>11} "
    ...
    for M in (2, 4, 8):
        for seed in (1, 2, 3):
            ...
            r = mc / max(mt, 1e-12)
            ratios.append(r)
            print(f"{M:>3} {2:>3} {seed:>4} {mse_f:>11.3e} "
