...
def main():
    ...
    if shutil.which("rustc") is None:
        sys.exit("error: rustc required (rustup target add wasm32-wasip1)")
    ...
    rows = []
    print(
        f"Benchmark: rclite wasm32-wasip1 (wasmtime, f32) "
        f"vs rclite host LLVM JIT (f64)"
    )
    print(
        f"Inference: T={X_te.shape[0]} samples on Mackey-Glass; "
    ...
    for topology, N in cases:
        ...
        row = run_one(
            topology, N, X_tr, Y_tr, X_te, Y_te, input_offset, wasmtime_bin
        )
        rows.append(row)
        print(
            f"host={row['host_best_ms']:.3f}ms wasm={row['wasm_best_ms']:.3f}ms "
    ...
    print("-" * 100)
    print(
        f"{'case':<22} {'host_best':>10} {'wasm_best':>10} "
    ...
    print("-" * 100)
    print(
        f"{'case':<22} {'host .so':>10} {'host .text':>12} "
        f"{'wasm kernel.o':>15} {'wasm full':>11}"
    )
    print(
        f"{'':<22} {'(weights)':>10} {'(code only)':>12} "
    ...
    print("-" * 100)
    print(
        f"{'case':<22} {'parity max':>14} {'host RMSE':>11} "
