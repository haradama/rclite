...
def eval_sym_qat(
    rc, exe, X, Y, eval_Y, sig_std, target, sf_range, ifrac, wfrac, lut_n
):
    res = search_quantization(
        ...
        X[:N_TRAIN],
        Y[:N_TRAIN],
        X[N_TRAIN : N_TRAIN + N_EVAL],
        Y[N_TRAIN : N_TRAIN + N_EVAL],
        target=target,
...
def eval_affine_1pass(
    rc, exe, X, eval_Y, sig_std, storage_bits, strategy=None
):
...
def main():
    seeds = (42, 7, 123)
    print(
        f"Mackey-Glass 1-step-ahead | N={RESERVOIR_N} SCR | "
    ...
    for seed in seeds:
        ...
        table_a.setdefault("i8 + i16 W_out (QAT)", []).append(
            eval_affine_qat(
                rc, exe, X, Y, eval_Y, sig_std, 8, w_out_storage_bits=16
            )
        )
