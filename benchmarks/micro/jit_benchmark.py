...
def build_esn(units: int, input_offset: float,
              topology: Topology = Topology.ESN_STANDARD) -> ReservoirComputer:
...
def main() -> None:
    ...
    print(f"{'topology':<10} {'units':>6}  {'numpy [ms]':>12}  {'jit [ms]':>12}  "
          f"{'speedup':>8}  {'compile [ms]':>14}  {'max |diff|':>12}")
    for topo in (Topology.ESN_STANDARD, Topology.SCR, Topology.DLR, Topology.DLRB):
        for N in [100, 500, 1000]:
            ...
            print(f"{topo.name:<10} {N:>6}  {t_np * 1000:>12.2f}  "
