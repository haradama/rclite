...
def main() -> int:
    ...
    if rc.readout.include_bias:
        w_bias = W_out[:, 0:1]; off = 1
    if rc.readout.include_input:
        w_in_out = W_out[:, off:off + K]; off += K
    ...
    np.savez(
        ...
        W_out_bias=(w_bias if w_bias is not None else np.zeros((W_out.shape[0], 0))),
        W_out_input=(w_in_out if w_in_out is not None else np.zeros((W_out.shape[0], 0))),
