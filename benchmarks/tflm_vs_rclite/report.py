...
def main() -> int:
    ...
    a("")
    a(
        f"Board: **{fw['board']}** (qemu `{fw['qemu_machine']}`). "
        f"Toolchain: {fw['toolchain']}."
    )
    a(
        f"Task: Mackey-Glass one-step-ahead prediction "
        f"({rc['n_test']} held-out targets). The **same 80-unit ESN** is deployed "
        f"two ways — apples-to-apples, isolating the deployment stack."
    )
    a("")
    a(
        "> **Why Cortex-M0 and not Arduino Uno?** LiteRT for Microcontrollers / "
    ...
    a("")
    a(
        "The **identical 80-unit ESN** (same float weights), deployed two ways: "
    ...
    a("|---|--:|--:|--:|--:|:--|")
    a(
        f"| TFLM ESN cell | **{_kb(te['flash'])}** | **{te['ram']} B** "
    ...
    a("")
    a(
        "¹ qemu `-icount` instruction estimate (NOT silicon cycles), per one "
        "prediction step."
    )
    a("")
    ...
    a("|---|--:|")
    a(
        f"| persistence baseline (s[t+1]≈s[t]) | {rc['nrmse_persistence_test'] * 100:.1f}% |"
    )
    a(f"| ESN float (reference) | {rc['nrmse_float_test'] * 100:.2f}% |")
    a(
        f"| TFLM ESN cell **int8 PTQ** (deployed) | {esn['nrmse_int8_test'] * 100:.0f}% |"
    )
    a(f"| rclite ESN **i8 PTQ** | {q['i8_affine_ptq'] * 100:.0f}% |")
    a(
        f"| rclite ESN **i8 QAT** (deployed) | {q['i8_affine_qat'] * 100:.2f}% |"
    )
    a(f"| rclite ESN i8 + i16 W_out QAT | {q['i8_i16wout_qat'] * 100:.2f}% |")
    a(
        f"| rclite ESN **i16 QAT** (deployed) | {q['i16_affine_qat'] * 100:.2f}% |"
    )
    a("")
    a(
        "Naive **int8 PTQ collapses for both** stacks on this chaotic regression "
    ...
    a("")
    a(
        f"Model: {rc['topology']} reservoir, {rc['reservoir_units']} units "
