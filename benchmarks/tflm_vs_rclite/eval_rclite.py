...
def main() -> int:
    ...
    qm8_ptq = quantize_model_affine(
        rc, exe, cfg8, lut_strategy=LUTStrategy.linear_interp(64)
    )
    ...
    K, M = 1, 1
    F = (
        (1 if rc.readout.include_bias else 0)
        + (K if rc.readout.include_input else 0)
        + N_UNITS
    )
    n_w_in = N_UNITS * K
    ...
    best_qm = {
        "i8_affine_qat": res8.best_qmodel,
        "i8_i16wout_qat": res_mix.best_qmodel,
        "i16_affine_qat": res16.best_qmodel,
        "i8_affine_ptq": qm8_ptq,
    }[best_name]
    bexe = AffineQuantizedExecutor(best_qm)
    bexe.reset()
