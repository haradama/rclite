...
def _build_and_quant(
    storage_bits=8, lut_strategy=None, units=20, T=300, seed=0
):
...
def test_build_direct_artifacts_full_table():
    _, _, qm, _ = _build_and_quant(
        storage_bits=8, lut_strategy=LUTStrategy.direct()
    )
...
def test_build_polynomial_table_is_empty():
    _, _, qm, _ = _build_and_quant(
        storage_bits=8,
        lut_strategy=LUTStrategy.polynomial(),
    )
...
def test_executor_direct_i8():
    _check_executor_runs(LUTStrategy.direct(), 8)


def test_executor_direct_i16():
    _check_executor_runs(LUTStrategy.direct(), 16)


def test_executor_interp_i8():
    _check_executor_runs(LUTStrategy.linear_interp(64), 8)


def test_executor_interp_i16():
    _check_executor_runs(LUTStrategy.linear_interp(256), 16)


def test_executor_poly_i8():
    _check_executor_runs(LUTStrategy.polynomial(), 8)


def test_executor_poly_i16():
    _check_executor_runs(LUTStrategy.polynomial(), 16)
...
def _check_parity(strategy, sb, topology=Topology.SCR):
    ...
    assert diff == 0.0, (
        f"strategy={strategy.kind}, sb={sb}: JIT vs Py diff = {diff}"
    )


def test_parity_direct_i8():
    _check_parity(LUTStrategy.direct(), 8)


def test_parity_direct_i16():
    _check_parity(LUTStrategy.direct(), 16)


def test_parity_interp_i8():
    _check_parity(LUTStrategy.linear_interp(64), 8)


def test_parity_interp_i16():
    _check_parity(LUTStrategy.linear_interp(256), 16)


def test_parity_polynomial_i8():
    _check_parity(LUTStrategy.polynomial(), 8)


def test_parity_polynomial_i16():
    _check_parity(LUTStrategy.polynomial(), 16)


def test_parity_interp_dense():
    _check_parity(
        LUTStrategy.linear_interp(128), 8, topology=Topology.ESN_STANDARD
    )
...
def test_microbit_cross_compile_i16_interp_size_smaller_than_direct():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        a_d = microbit.compile_affine_quantized(
            qm_d,
            output_dir=tmp / "direct",
            test_inputs=X[200:210],
        )
        _, _, qm_i, _ = _build_and_quant(
            storage_bits=16,
            lut_strategy=LUTStrategy.linear_interp(n_entries=256),
        )
        a_i = microbit.compile_affine_quantized(
            qm_i,
            output_dir=tmp / "interp",
            test_inputs=X[200:210],
        )
...
def test_structured_jit_parity_per_topology():
    for topo in (Topology.SCR, Topology.DLR, Topology.DLRB):
        for sb in (8, 16):
            ...
            diff = float(np.max(np.abs(Y_jit - Y_py)))
            assert diff == 0.0, f"{topo.name} i{sb}: JIT vs Py diff = {diff}"
...
def test_structured_topology_smaller_mcu_binary():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp = pathlib.Path(tmp)
        _, _, qm_scr, X = _build_and_quant_topo(
            Topology.SCR, units=60, storage_bits=8
        )
        ...
        _, _, qm_dense, _ = _build_and_quant_topo(
            Topology.ESN_STANDARD, units=60, storage_bits=8
        )
        a_dense = microbit.compile_affine_quantized(
            qm_dense,
            output_dir=tmp / "dense",
            test_inputs=X[150:160],
        )
...
def test_preprocess_ir_emits_PreprocessInput_op_only_when_needed():
    ...
    assert not has_preprocess(mod_id), (
        "identity preprocess should NOT emit PreprocessInput"
    )
    assert has_preprocess(mod_aff), (
        "non-identity preprocess should emit PreprocessInput"
    )
