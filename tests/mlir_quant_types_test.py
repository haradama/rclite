...
from rclite.codegen.mlir_quant_types import emit_quant_types, uniform_type, verify
...
def _qm(pcr=False, pco=False, topology=Topology.ESN_STANDARD):
    rc = ReservoirComputer(
        ...
        readout=ReadoutNode(units=3, trainer=Trainer.RIDGE,
...
def test_per_tensor_verifies():
    if not HAVE:
        print("  (skip: mlir-opt not on PATH)"); return
...
TESTS = [test_uniform_type_strings, test_per_tensor_verifies,
