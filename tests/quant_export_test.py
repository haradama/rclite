...
def build_symmetric(topology, target_factory, cfg, lut_n, **rc_kw):
    ...
    exe, X = _train(rc)
    qm = quantize_model(
        rc, exe, cfg, target=target_factory(), lut=TanhLUTSpec(n=lut_n)
    )
    return qm, X
...
def sym_q_x(qm, X_float):
    ...
    return np.array(
        [
            qm.target.quantize_input_array(X_float[t], cfg)
            for t in range(X_float.shape[0])
        ],
        dtype=np.int64,
    )
...
def test_sym_host_parity_i16():
    qm, X = build_symmetric(
        Topology.SCR,
        I16FixedPoint,
        QuantConfig(state_frac=10, input_frac=8, weight_frac=8),
        128,
    )
    _assert_sym_host_parity(qm, X[150:175])
...
def test_sym_host_parity_i32():
    qm, X = build_symmetric(
        Topology.ESN_STANDARD,
        I32FixedPoint,
        QuantConfig(state_frac=16, input_frac=14, weight_frac=14),
        256,
    )
    _assert_sym_host_parity(qm, X[150:175])
