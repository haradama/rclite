...
def _model(
    ...
    rc = ReservoirComputer(
        input=InputNode(
            units=K, input_offset=off, input_scaling=sc, name="in"
        ),
...
def _qm(rc, exe, X, sb=8, **kw):
    ...
    return (
        quantize_model_affine(rc, exe, cfg, lut_strategy=lut)
        if lut
