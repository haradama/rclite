...
def _build_and_train(**overrides):
    cfg = dict(
        units=80, topology=Topology.ESN_STANDARD,
        spectral_radius=0.9, leak_rate=0.3, density=0.2,
    ...
    rc = ReservoirComputer(
        ...
        readout=ReadoutNode(
            units=1, trainer=Trainer.RIDGE, regularization=1e-6, washout=100,
            include_bias=cfg["include_bias"], include_input=cfg["include_input"],
            name="out",
        ),
    )
    exe = RCExecutor(rc)
...
def _assert_close(a: np.ndarray, b: np.ndarray, atol: float = 1e-10,
                  rtol: float = 1e-10):
...
def test_parity_dlr_structured():
    rc, exe, X = _build_and_train(
        topology=Topology.DLR, chain_weight=0.8,
        input_distribution=Distribution.BERNOULLI,
    )
    Y_np = exe.predict(X)
    Y_jit = compile_rc(rc, exe).predict(X)
...
def test_parity_dlrb_structured():
    rc, exe, X = _build_and_train(
        topology=Topology.DLRB, chain_weight=0.7, chain_feedback=0.1,
        input_distribution=Distribution.BERNOULLI,
    )
    Y_np = exe.predict(X)
    Y_jit = compile_rc(rc, exe).predict(X)
...
def test_parity_multi_dim_output():
    rng = np.random.default_rng(0)
    rc = ReservoirComputer(
        input=InputNode(units=2, input_offset=0.0, input_scaling=1.0, name="in"),
        reservoir=ReservoirNode(units=60, spectral_radius=0.9, leak_rate=0.5,
                                density=0.3, seed=42, name="res"),
        readout=ReadoutNode(units=3, trainer=Trainer.RIDGE, regularization=1e-6,
                            washout=80, include_bias=True, include_input=True,
                            name="out"),
    )
    exe = RCExecutor(rc)
