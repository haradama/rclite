...
from rclite import (
    ...
    Activation, Distribution, Topology, Trainer, Task, Aggregation,
)
...
def _build(agg, *, topo=Topology.SCR, units=32, sb=8, wob=16, M=4, washout=8,
           seed=0):
    rc = ReservoirComputer(
        ...
        reservoir=ReservoirNode(units=units, activation=Activation.TANH,
                                 topology=topo, chain_weight=0.9,
                                 chain_feedback=0.1, leak_rate=0.3, seed=7),
        readout=ReadoutNode(units=M, activation=Activation.IDENTITY,
                            trainer=Trainer.RIDGE, regularization=1e-2,
                            washout=washout, include_bias=True,
                            task=Task.CLASSIFICATION, aggregation=agg),
    )
    exe = RCExecutor(rc)
    ...
    cfg = calibrate_from_data(rc, exe, np.concatenate(seqs),
                              storage_bits=sb, w_out_storage_bits=wob)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        r = subprocess.run(
            ["gcc", "-O2", "-std=c99", "-o", str(td / "a.out"),
             str(td / "m.c"), str(td / "k.c")],
            capture_output=True, text=True)
        if r.returncode != 0:
            raise RuntimeError("gcc failed:\n" + r.stderr)
        out = subprocess.run([str(td / "a.out")],
...
def test_pooling_quant_accuracy_reasonable():
    ...
    agree = 0
    for X in seqs:
        f = int(np.argmax(exe.predict_sequences([X])[:1]) if False
...
def test_pooling_rejects_include_input():
    ...
    rc = ReservoirComputer(
        ...
        reservoir=ReservoirNode(units=16, topology=Topology.SCR,
                                 chain_weight=0.9, leak_rate=0.3, seed=1),
        readout=ReadoutNode(units=3, trainer=Trainer.RIDGE, regularization=1e-2,
                            washout=5, include_bias=True, include_input=True,
