...
from xdsl.dialects.builtin import (
    ModuleOp, IntegerType, IndexType, MemRefType, TensorType,
    DenseIntOrFPElementsAttr, StringAttr, UnitAttr, IntegerAttr,
)
...
def _dense_global(name, arr, bits) -> memref.GlobalOp:
    ...
    return memref.GlobalOp.get(StringAttr(name), ty, init,
                               sym_visibility=StringAttr("private"),
                               constant=UnitAttr())


def emit_affine_mlir_xdsl(qmodel: AffineQuantizedModel, *,
    ...
    inc_b, inc_i = bool(rc.readout.include_bias), bool(rc.readout.include_input)
    ...
    if structured:
        ...
        cw_q, cf_q = _qchain(rc.reservoir.chain_weight), _qchain(rc.reservoir.chain_feedback)
    if use_sparse:
    ...
    if inc_i:
        globals_.append(_dense_global("rs_out_i", qmodel.row_sum_Wout_input, 32))
    if not structured:
        if use_sparse:
            globals_ += [_dense_global("Wres_val", wr_val, sb),
    ...
    def rq_func(name, M0, n):
        ...
        funcs.append(func.FuncOp(name, ([_I32], [_I32]), r, visibility="private"))
    ...
    with ImplicitBuilder(r.block) as (x,):
        b = arith.MinSIOp(arith.MaxSIOp(x, c_i(-2147483648, _I64)).result,
                          c_i(2147483647, _I64)).result
        func.ReturnOp(arith.TruncIOp(b, _I32).result)
    funcs.append(func.FuncOp("clip32", ([_I64], [_I32]), r, visibility="private"))

    # ---- sat: i32 -> storage (saturate) ----
    r = Region([Block(arg_types=[_I32])])
    with ImplicitBuilder(r.block) as (x,):
        b = arith.MinSIOp(arith.MaxSIOp(x, c_i(qmin, _I32)).result,
                          c_i(qmax, _I32)).result
    ...
    with ImplicitBuilder(r.block) as (p,):
        if lut_kind == LUTKind.DIRECT:
            ...
            idx32 = arith.AddiOp(arith.ExtSIOp(p, _I32).result,
                                 c_i(qmodel.lut_offset, _I32)).result
            ...
            lut = memref.GetGlobalOp("lut", MemRefType(isb, [lut_size])).memref
            norm = arith.AddiOp(arith.ExtSIOp(p, _I32).result,
                                c_i(art.offset, _I32)).result
            t = call("rq_lidx", [norm], _I32)
            idxr = arith.ShRSIOp(t, c_i(f, _I32)).result
            idxc = arith.MinSIOp(arith.MaxSIOp(idxr, c_i(0, _I32)).result,
                                 c_i(ne - 2, _I32)).result
            frac = arith.SubiOp(t, arith.ShLIOp(idxc, c_i(f, _I32)).result).result
            ...
            sc = arith.TruncIOp(arith.ShRSIOp(mul, c_i(f, _I64)).result, _I32).result
            interp = arith.AddiOp(y0, sc).result
            func.ReturnOp(call("sat", [interp], isb))
        else:  # POLYNOMIAL
            qf = strat.poly_qf_bits
            cent = arith.SubiOp(arith.ExtSIOp(p, _I32).result,
                                c_i(zp_pre, _I32)).result
            xq32 = call("rq_polyx", [cent], _I32)
            xq = arith.MinSIOp(
                arith.MaxSIOp(ext(xq32), c_i(-art.x_clip_qf, _I64)).result,
                c_i(art.x_clip_qf, _I64)).result
            qfc = c_i(qf, _I64)
            x2 = arith.ShRSIOp(arith.MuliOp(xq, xq).result, qfc).result
            m5 = arith.ShRSIOp(arith.MuliOp(x2, c_i(art.poly_a5_qf, _I64)).result, qfc).result
            ...
            yq = arith.MinSIOp(arith.MaxSIOp(yq0, c_i(-art.one_qf, _I64)).result,
                               c_i(art.one_qf, _I64)).result
    ...
    funcs.append(func.FuncOp("activate", ([isb], [isb]), r, visibility="private"))
    ...
    with ImplicitBuilder(main_r.block) as (T, X, Y):
        ...
        rsOI = (memref.GetGlobalOp("rs_out_i", MemRefType(_I32, [M])).memref
                if inc_i else None)
        if not structured:
            rsRes = memref.GetGlobalOp("rs_res", MemRefType(_I32, [N])).memref
            if use_sparse:
                WrV = memref.GetGlobalOp("Wres_val", MemRefType(isb, [wr_val.size])).memref
                WrC = memref.GetGlobalOp("Wres_col", MemRefType(_I32, [wr_col.size])).memref
                WrP = memref.GetGlobalOp("Wres_rptr", MemRefType(_I32, [wr_rptr.size])).memref
            else:
                Wres = memref.GetGlobalOp("W_res", MemRefType(isb, [N * N])).memref
        SM = (memref.GetGlobalOp("sm_lut", MemRefType(isb, [sm_size])).memref
              if proba else None)
        ...
        logits = memref.AllocaOp.get(isb, shape=[M]).memref if has_logits_buf else None
        exps = memref.AllocaOp.get(_I32, shape=[M]).memref if proba else None

        def init_body(i, _):
            memref.StoreOp.get(zps, h, [i]); return []
        ...
        def acc_res(i):
            if structured and topo == Topology.SCR:
                ...
                iprev = arith.SelectOp(iz, c_idx(N - 1), arith.SubiOp(i, c1).result).result
                hv32 = arith.ExtSIOp(memref.LoadOp.get(h, [iprev]).res, _I32).result
                ...
                isafe = arith.SelectOp(ipos, arith.SubiOp(i, c1).result, c0).result
                hv32 = arith.ExtSIOp(memref.LoadOp.get(h, [isafe]).res, _I32).result
                hc = arith.SubiOp(hv32, zps32).result
                ...
                nm1 = c_idx(N - 1)
                ipos = arith.CmpiOp(i, c0, "sgt").result
                ib = arith.SelectOp(ipos, arith.SubiOp(i, c1).result, c0).result
                hb32 = arith.ExtSIOp(memref.LoadOp.get(h, [ib]).res, _I32).result
                ...
                iff = arith.SelectOp(ilt, arith.AddiOp(i, c1).result, nm1).result
                hf32 = arith.ExtSIOp(memref.LoadOp.get(h, [iff]).res, _I32).result
                ...
                if use_sparse:
                    rp0i = arith.IndexCastOp(memref.LoadOp.get(WrP, [i]).res, _IDX).result
                    rp1i = arith.IndexCastOp(
                        memref.LoadOp.get(WrP, [arith.AddiOp(i, c1).result]).res, _IDX).result

                    def pbody(p, args):
                        ar, = args
                        w = memref.LoadOp.get(WrV, [p]).res
                        cj = arith.IndexCastOp(memref.LoadOp.get(WrC, [p]).res, _IDX).result
        ...
        def time_body(t, _):
            ...
            if int_pre:
                def pre_in(k, _):
                    ...
                    cent = arith.SubiOp(arith.ExtSIOp(xq, _I32).result,
                                        c_i(zp_input, _I32)).result
                    d = call("rq_pre", [cent], _I32)
                    tot = arith.AddiOp(d, c_i(qmodel.pre_const, _I32)).result
                    memref.StoreOp.get(call("sat", [tot], isb), upre, [k]); return []
            ...
            def pre_body(i, _):
                ...
                memref.StoreOp.get(call("sat", [s2], isb), pre, [i]); return []
            ...
            def act_body(i, _):
                ...
                hold32 = arith.ExtSIOp(memref.LoadOp.get(h, [i]).res, _I32).result
                ...
                memref.StoreOp.get(call("sat", [nh], isb), h, [i]); return []
            ...
            def ro_body(m, _):
                ...
                zpo = c_i(zp_out, _I32)
                if inc_b:
                    w0 = memref.LoadOp.get(Wout, [arith.AddiOp(mF, c_idx(off_b)).result]).res
                ...
                if inc_i:
                    ...
                    def kbody(k, args):
                        ao, = args
                        widx = arith.AddiOp(mF, arith.AddiOp(ci, k).result).result
                        w = memref.LoadOp.get(Wout, [widx]).res
                        xv = memref.LoadOp.get(X, [arith.AddiOp(tK, k).result]).res
                        return [arith.AddiOp(ao, arith.MuliOp(ext(w), ext(xv)).result).result]
                    accoi = for_(c0, cK, c1, [z64], kbody)[0]
                    rsoi64 = ext(memref.LoadOp.get(rsOI, [m]).res)
                    zroi = arith.MuliOp(c_i(zp_input, _I64), rsoi64).result
                    coi = call("clip32", [arith.SubiOp(accoi, zroi).result], _I32)
                ...
                def jbody(j, args):
                    ...
                    w = memref.LoadOp.get(Wout, [widx]).res
                    hv = memref.LoadOp.get(h, [j]).res
                    return [arith.AddiOp(ao, arith.MuliOp(ext(w), ext(hv)).result).result]
            ...
            if classify:
                ...
                def amax(m, args):
                    ...
                    return [arith.SelectOp(gt, v, bv).result,
                ...
                def sbody(m, args):
                    ...
                    d = arith.SelectOp(arith.CmpiOp(d0, dmin, "slt").result, dmin, d0).result
                ...
                def pbody(m, _):
                    ...
                    memref.StoreOp.get(pq, Y, [arith.AddiOp(tM, m).result]); return []
                for_(c0, cM, c1, [], pbody)
            return []
