...
def emit_symmetric_mlir_xdsl(qmodel: QuantizedModel, *,
    ...
    leak_q = int(qmodel.target.quantize_state(float(rc.reservoir.leak_rate), cfg))
    ...
    with ImplicitBuilder(sat_r.block) as (x,):
        b = arith.MinSIOp(arith.MaxSIOp(x, c_i(qmin, _I32)), c_i(qmax, _I32)).result
    ...
    with ImplicitBuilder(act_r.block) as (p,):
        ...
        x = arith.MinSIOp(arith.MaxSIOp(x0, c_i(xmin_q, _I32)),
                          c_i(xmax_q, _I32)).result
        ...
        i0 = arith.MinSIOp(arith.MaxSIOp(i0r, c_i(0, _I32)),
                           c_i(lut_n - 2, _I32)).result
    ...
    act_fn = func.FuncOp("activate", ([isb], [isb]), act_r, visibility="private")
    ...
    with ImplicitBuilder(main_r.block) as (T, X, Y):
        ...
        if not structured:
            if use_sparse:
                WrV = memref.GetGlobalOp(
                    "Wres_val", MemRefType(isb, [wr_val.size])).memref
                WrC = memref.GetGlobalOp(
                    "Wres_col", MemRefType(_I32, [wr_col.size])).memref
                WrP = memref.GetGlobalOp(
                    "Wres_rptr", MemRefType(_I32, [wr_rptr.size])).memref
            else:
        ...
        def acc_res(i, accin):
            if structured and topo == Topology.SCR:
                ...
                iprev = arith.SelectOp(
                    iz, c_idx(N - 1), arith.SubiOp(i, c1).result).result
            ...
            if structured and topo == Topology.DLR:
                ...
                trsel = arith.SelectOp(ipos, fmul(cw, hv, shift_res), z32).result
            ...
            if structured:  # DLRB
                ...
                tbsel = arith.SelectOp(ipos, fmul(cw, hb, shift_res), z32).result
                ...
                tfsel = arith.SelectOp(ilt, fmul(cfk, hf, shift_res), z32).result
                return arith.AddiOp(arith.AddiOp(accin, tbsel).result, tfsel).result
            if use_sparse:
        ...
        def init_body(i, _):
            memref.StoreOp.get(zsb, h, [i]); return []
        ...
        def time_body(t, _):
            ...
            def pre_body(i, _):
                ...
                memref.StoreOp.get(arith.TruncIOp(accres, isb).result, pre, [i])
                return []
            ...
            def act_body(i, _):
                ...
                t1 = arith.TruncIOp(fmul(hold, c_i(one_ml_q, isb), sf), isb).result
                t2 = arith.TruncIOp(fmul(act, c_i(leak_q, isb), sf), isb).result
            ...
            def ro_body(m, _):
                ...
                if inc_b:
                    w0 = memref.LoadOp.get(Wout, [arith.AddiOp(mF, c0).result]).res
                ...
                cl = arith.MinSIOp(arith.MaxSIOp(shifted, c_i(qmin, _I64)),
                                   c_i(qmax, _I64)).result
