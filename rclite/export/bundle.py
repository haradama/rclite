...
def _emit_kernel_and_info(qmodel, name: str, head=None, sparse=None):
    ...
    if isinstance(qmodel, AffineQuantizedModel):
        ...
        return (
            emit_affine_kernel_c(qmodel, head=head, sparse=sparse),
    ...
    if isinstance(qmodel, QuantizedModel):
        ...
        return (
            emit_symmetric_kernel_c(qmodel, head=head, sparse=sparse),
...
def _readme(info: KernelInfo) -> str:
    ...
    if info.is_classify:
        ...
        out_comment = (
            "/* float out: p = rc_dequantize_output(Y[i]); */"
            if info.is_proba
...
def export_bundle(
    qmodel, out_dir, *, name: str = "rc_model", head=None, sparse=None
) -> pathlib.Path:
    ...
    kernel_c, info = _emit_kernel_and_info(
        qmodel, name, head=head, sparse=sparse
    )
