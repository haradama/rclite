...
// f32, runs the inference REPEATS+WARMUP times via `std::time::Instant`
// (WASI clock), and prints the best/median/mean wall-clock time per call
// plus a parity report against the embedded reference.
//
// Output is keyed (lines starting with "RCLITE_BENCH:") so the Python
// driver can parse the numbers back without depending on float-formatting
// roundtrips.
//
// Template placeholders (filled in by WasmTarget.compile_bench):
//   @@T@@          -- number of inference timesteps
//   @@K@@          -- input units
//   @@M@@          -- output units
//   @@REPEATS@@    -- measured repetitions
