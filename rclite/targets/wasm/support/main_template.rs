// Wasmtime harness for the cross-compiled rc_predict.
//
// Compares the wasm32 (f32) prediction against an f64 host reference cast
// to f32 at build time. Output is written via stdout (WASI fd_write).
//
// Template placeholders (filled in by rclite.targets.wasm.WasmTarget):
//   @@T@@          -- number of inference timesteps
//   @@K@@          -- input units
//   @@M@@          -- output units
//   @@X_VALUES@@   -- comma-separated input samples (T*K floats)
//   @@Y_VALUES@@   -- comma-separated host-reference predictions (T*M floats)
