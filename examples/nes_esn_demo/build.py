...
from rclite.ir import build_ir, VerifyEchoStateConstraint
...
def train():
    ...
    # Surface stability diagnostics before quantization/lowering.
    irm = build_ir(rc, exe)
    irm = VerifyEchoStateConstraint(strict=False)(irm)
    warns = irm.metadata.get("echo_state_warnings", [])
    if warns:
        print(f"[train] stability warnings: {len(warns)}")
        print(f"[train] first warning: {warns[0]}")
    else:
        print("[train] stability check: OK")
...
def emit_data_header(qm, eval_u, eval_truth, y_pred, path: pathlib.Path):
    ...
    truth_q = (
        cfg.output.quantize_array(eval_truth[:, None]).astype(np.int16).ravel()
    )
...
def build_rom(qm, eval_u, eval_truth, y_pred):
    ...
    if shutil.which(cc) is None:
        print(
            f"[build] {cc} not found — install the llvm-mos SDK and put its "
...
def screenshot(rom: pathlib.Path, out_png: pathlib.Path, timeout=180):
    ...
    if fceux is None:
        print(
            "[shot] fceux not found — install it (`apt install fceux`) to "
