...
def train():
    ...
    qm = quantize_model_affine(rc, exe, cfg,
                               lut_strategy=LUTStrategy.linear_interp(256))
...
def build_rom(qm, eval_u, eval_truth):
    ...
    subprocess.run([cc, "-c", *cpu, "-marm", str(BUILD / "crt0.s"),
                    "-o", str(BUILD / "crt0.o")], check=True)
    subprocess.run([cc, "-c", *cpu, "-mthumb", "-O2", f"-I{BUILD}",
                    str(BUILD / "main.c"), "-o", str(BUILD / "main.o")],
                   check=True)
    elf = BUILD / "esn_demo.elf"
    subprocess.run([cc, *cpu, "-mthumb", "-T", str(BUILD / "gba.ld"),
                    "-nostartfiles", "-Wl,--gc-sections", "--specs=nosys.specs",
                    str(BUILD / "crt0.o"), str(BUILD / "main.o"), str(rc_o),
                    "-o", str(elf), "-lgcc", "-lc", "-lnosys"], check=True)
    rom = BUILD / "esn_demo.gba"
    subprocess.run([cc.replace("gcc", "objcopy"), "-O", "binary",
                    str(elf), str(rom)], check=True)
...
def reconstruct_png(out: str, path: pathlib.Path):
    lines = [l.split("GBA Debug: ", 1)[1] for l in out.splitlines()
             if "GBA Debug:" in l]
    ...
    W, H = 240, 160
    pal = np.array([[0, 0, 0], [255, 255, 255], [0, 220, 0], [230, 0, 0]],
                   dtype=np.uint8)
    ...
    _write_png(img, path)
    print(f"[verify] reconstructed screen -> {path}  "
