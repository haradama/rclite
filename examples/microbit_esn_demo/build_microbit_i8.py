...
def parse_size(elf_path: pathlib.Path, cc: str) -> dict:
    ...
    cp = subprocess.run(
        [size_cmd, str(elf_path)], capture_output=True, text=True, check=True
    )
...
def main() -> None:
    ...
    for label, factory, sf_range, ifrac, wfrac, lut_n in _RECIPES:
        print(
            f"[2/3] QAT + cross-compile: storage={label} "
        ...
        print(
            f"      best state_frac={qmodel.config.state_frac}, "
    ...
    print("\n" + "=" * 66)
    print(
        f"{'storage':<8} {'state_frac':>10} {'text':>8} {'data':>6} "
        f"{'bss':>6} {'total':>8} {'MSE':>14}"
    )
    print("-" * 66)
    for label, qm, sizes, mse, _, _ in rows:
        print(
            f"{label:<8} {qm.config.state_frac:>10} "
    ...
    for label, _, sizes, _, _, _ in rows[1:]:
        ...
        print(
            f"  {label} vs i32: {sign}{abs(delta):>5} bytes "
    ...
    print(
        f"[3/3] qemu-system-arm run on i8 binary "
