...
def build(input_offset: float, *, spectral_radius: float, leak_rate: float,
          input_scaling: float, regularization: float, seed: int = 42,
          units: int = 500) -> ReservoirComputer:
    return ReservoirComputer(
        input=InputNode(
            units=1, activation=Activation.IDENTITY,
            input_scaling=input_scaling, input_offset=input_offset,
            name="input",
        ),
        ...
        readout=ReadoutNode(
            units=1, activation=Activation.IDENTITY,
            trainer=Trainer.RIDGE, regularization=regularization,
...
def main() -> None:
    ...
    test_target = series[test_seed_start + 200:
                         test_seed_start + 200 + test_horizon, None]
    ...
    for seed in [1, 7, 17, 42, 99]:
        ...
        req = WellPosedReservoir(
            ...
            empirical_check=InputDrivenESPCheck(
                executor=exe, sample_input=X_tr, threshold=0.0,
            ),
        )
        try:
            req.check()
