...
def main() -> None:
    ...
    Y_q = qexe.predict(np.concatenate([X[:n_train], sample], axis=0))[
        -len(sample) :
    ]
    ...
    if "flash_bytes" in md:
        print(
            f"      Flash: {md['flash_bytes']} bytes / 32768  "
            f"({md['flash_bytes'] / 32768 * 100:.1f}%)"
        )
        print(
            f"      SRAM : {md['sram_bytes']} bytes / 2048   "
        ...
        print(
            "      arduino-cli not on PATH — sketch emitted but not compiled."
        )
